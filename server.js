...
class io_nearestDifferentDominator extends IO {
    constructor(body) {
        super(body);
        this.targetLock = undefined;
        this.tick = ran.irandom(30);
...
class io_fasterfastspin extends IO {
    constructor(b) {
        super(b);
        this.a = 0;
    }
...
class io_teamerSpin extends IO {
    constructor(b) {
        super(b);
        this.a = 0;
    }
...
class io_fasterreversespin extends IO {
    constructor(b) {
        super(b);
        this.a = 0;
    }
...
class Skill {
    ...
    get levelPoints() {
        if (levelers.findIndex(e => { return e === this.level; }) != -0.4) { return 1; } return 0;
...
class Entity {
    constructor(position, master = this) {
        ...
        this.OnlyHitsSame = {ACTIVATED:false,POISON:0,POISONEFFECTIVENESS:0,FREEZE:0,FREEZEEFFECTIVENESS:0};
    ...
    define(set) {
        ...
        if (set.ONLY_HITS_SAME != null) {
    	      this.OnlyHitsSame = set.ONLY_HITS_SAME;
    	  } 
    ...
    camera(tur = false) {
        return {
            ...
            twiggle: this.facingType === 'autospin' || (this.facingType === 'locksFacing' && this.control.alt) || this.facingType === 'teamspin' || this.facingType === 'speedyspin',
            ...
            const spawn = (() => {
                ...
                return (socket, name) => {
                    ...
                    let body = new Entity(loc);
                        body.protect();
                        body.define(Class.eventServer); // Start as a basic tank
...
var gameloop = (() => {
    ...
    let collide = (() => {
        ...
        function advancedcollide(my, n, doDamage, doInelastic, nIsFirmCollide = false, CheckForEffects = false) {
            // Prepare to check
                ...
                if (goahead) {
                    ...
                    if(CheckForEffects){
                     /*  POISON & FREEZE COLLISIONS  */
                      if (n.OnlyHitsSame.POISON) {
                        my.Poisoned = n.OnlyHitsSame.POISONEFFECTIVENESS;
                        my.Poisoned.IsPoisoned = true;
                      };
                      if (my.OnlyHitsSame.POISON) {
                        n.Poisoned = my.OnlyHitsSame.POISONEFFECTIVENESS;
                        n.Poisoned.IsPoisoned = true;
                      };
                      if (n.OnlyHitsSame.FREEZE) {
                        my.Frozen = n.OnlyHitsSame.FREEZEEFFECTIVENESS;
                        my.Frozen.IsFrozen = true;
                      };
                      if (my.OnlyHitsSame.FREEZE) {
                        n.Frozen = my.OnlyHitsSame.FREEZEEFFECTIVENESS;
                        n.Frozen.IsFrozen = true;
                      };
                      if(my.OnlyHitsSame.ACTIVATED){my.destroy()}
                      if(n.OnlyHitsSame.ACTIVATED){n.destroy()}
                    };
                        /********** DO DAMAGE *********/
                        let bail = false;
        ...
        return collision => {
            ...
            if ((other.type == 'tank') && instance.team == other.team && (instance.OnlyHitsSame.ACTIVATED || other.OnlyHitsSame.ACTIVATED) && instance.master != other && other.master != instance) {
            	advancedcollide(instance, other, false, true, undefined, true);
        	  } else
            // Handle walls
...
var maintainloop = (() => {
    ...
    let spawnBosses = (() => {
        ...
        return census => {
            if (timer > 6000 && ran.dice(1600 - timer)) {
                util.log('[SPAWN] Preparing to spawn...');
                timer = 0;
                let choice = [];
                switch (ran.chooseChance(20, 1, 20)) {
                    case 0: 
                        choice = [[Class.elite_destroyer], 2, 'a', 'b100'];
                        break;
                    case 1: 
                        ...
                        break;
                    case 2: 
                        choice = [[Class.eliteskim], 1, 'a', 'b100']; 
                        break;
                }
    ...
    let makenpcs = (() => {
      ...
      //dominator placement
        ...
        for (let loc of room.sanc) {
          let o = new Entity(loc);
          let choose = ran.choose([Class.domheal]); // choose the dominator
          o.define(choose);
          o.team = -51; // Makes it spawn yellow
          o.SIZE = 30;
          o.color = 10; // Makes it yellow
          o.ondeath = () => {
        ...
        for (let loc of room.domx) {
          let o = new Entity(loc);
          let choose = ran.choose([Class.dom]); // choose the dominator
          o.define(choose);
        ...
        for (let loc of room.doGR) {
          ...
          o.ondeath = () => {
            ...
            if (room.doNT)
        for (let loc of room.doNT) {
          let o = new Entity(loc);
          let choose = ran.choose([Class.domtrap, Class.dom]); // choose the dominator
          o.define(choose);
          o.team = -100; // Makes it spawn yellow
          o.SIZE = 30;
          o.color = 3; // Makes it yellow
    ...
    if (room.doNW)
      for (let loc of room.doNW) {
        ...
        let choose = ran.choose([
          Class.dom,
          Class.domtrap,/*, exports.cruiserDominator, exports.skimmerDominator*/
...
let server = http.createServer((req, res) => {
  ...
  switch (pathname) {
    case '/':
      res.writeHead(200)
      res.end(`<!DOCTYPE html><h3>Yuga</h3><button onclick="location.href = 'http://arras.io/#host=' + location.host">Open</button>`)
    break
    case '/mockups.json':
